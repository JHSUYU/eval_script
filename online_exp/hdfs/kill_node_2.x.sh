...
# HDFS故障注入脚本 - Hadoop 2.x版本
# 随机选择单个节点kill DataNode
# 用于测试HDFS集群的容错能力
...
get_process_pid() {
    ...
    ssh $SSH_OPTIONS ${SSH_USER}@${server} 2>/dev/null \
        "jps 2>/dev/null | grep '$process_name' | awk '{print \$1}' | head -1"
}
...
# 启动DataNode (Hadoop 2.x版本)
start_datanode() {
    local server=$1

    log ACTION "Starting DataNode on $server (Hadoop 2.x)"

    # 使用命令而不是heredoc来避免登录信息
    ssh $SSH_OPTIONS ${SSH_USER}@${server} 2>/dev/null "
        ...
        # Hadoop 2.x 启动方式
        # 方法1: 使用hadoop-daemon.sh脚本
        if [ -f \"\$HADOOP_HOME/sbin/hadoop-daemon.sh\" ]; then
            \$HADOOP_HOME/sbin/hadoop-daemon.sh start datanode >/dev/null 2>&1
            echo 'Started DataNode using hadoop-daemon.sh'
        # 方法2: 使用hdfs-daemon.sh脚本 (某些2.x版本)
        elif [ -f \"\$HADOOP_HOME/sbin/hdfs-daemon.sh\" ]; then
            \$HADOOP_HOME/sbin/hdfs-daemon.sh start datanode >/dev/null 2>&1
            echo 'Started DataNode using hdfs-daemon.sh'
        # 方法3: 直接后台运行
        else
            mkdir -p \$HADOOP_HOME/logs
            nohup \$HADOOP_HOME/bin/hdfs datanode > \$HADOOP_HOME/logs/datanode.out 2>&1 &
            echo 'Started DataNode in background'
        fi
...
# 启动NameNode (Hadoop 2.x版本)
start_namenode() {
    local server=$1

    log ACTION "Starting NameNode on $server (Hadoop 2.x)"
    ...
    ssh $SSH_OPTIONS ${SSH_USER}@${server} 2>/dev/null "
        ...
        # 方法1: 使用hadoop-daemon.sh脚本
        if [ -f \"\$HADOOP_HOME/sbin/hadoop-daemon.sh\" ]; then
            \$HADOOP_HOME/sbin/hadoop-daemon.sh start namenode >/dev/null 2>&1
            echo 'Started NameNode using hadoop-daemon.sh'
        # 方法2: 使用hdfs-daemon.sh脚本 (某些2.x版本)
        elif [ -f \"\$HADOOP_HOME/sbin/hdfs-daemon.sh\" ]; then
            \$HADOOP_HOME/sbin/hdfs-daemon.sh start namenode >/dev/null 2>&1
            echo 'Started NameNode using hdfs-daemon.sh'
        # 方法3: 直接后台运行
        else
            mkdir -p \$HADOOP_HOME/logs
            nohup \$HADOOP_HOME/bin/hdfs namenode > \$HADOOP_HOME/logs/namenode.out 2>&1 &
            echo 'Started NameNode in background'
        fi
...
# 停止DataNode (Hadoop 2.x版本)
stop_datanode() {
    local server=$1

    log ACTION "Stopping DataNode on $server (Hadoop 2.x)"
    ...
    ssh $SSH_OPTIONS ${SSH_USER}@${server} 2>/dev/null "
        ...
        # 方法1: 使用hadoop-daemon.sh脚本
        if [ -f \"\$HADOOP_HOME/sbin/hadoop-daemon.sh\" ]; then
            \$HADOOP_HOME/sbin/hadoop-daemon.sh stop datanode >/dev/null 2>&1
        # 方法2: 使用hdfs-daemon.sh脚本
        elif [ -f \"\$HADOOP_HOME/sbin/hdfs-daemon.sh\" ]; then
            \$HADOOP_HOME/sbin/hdfs-daemon.sh stop datanode >/dev/null 2>&1
        # 方法3: 直接kill进程
        else
            PID=\$(jps 2>/dev/null | grep DataNode | awk '{print \$1}')
            if [ ! -z \"\$PID\" ]; then
                kill \$PID 2>/dev/null
            fi
        fi
...
kill_and_restart_random_datanode() {
    ...
    if kill_process "$server" "DataNode"; then
        ...
        log WARN "DataNode not running on node$node_index ($server), starting it..."
        start_datanode "$server"
    fi

    # 使用全局变量传递节点索引，避免SSH输出干扰
    LAST_SELECTED_NODE=$node_index
}

# 显示集群状态
show_cluster_status() {
    log INFO "Current HDFS Cluster Status (Hadoop 2.x):"
    echo "----------------------------------------"
...
check_hadoop_version() {
    log INFO "Checking Hadoop version..."

    # 从第一个节点检测版本
    local version=$(ssh $SSH_OPTIONS ${SSH_USER}@${SERVERS[0]} \
        "$HADOOP_HOME/bin/hadoop version 2>/dev/null | head -1 | awk '{print \$2}'")

    if [[ $version == 2.* ]]; then
        log INFO "Detected Hadoop version: $version (2.x compatible)"
    elif [[ $version == 3.* ]]; then
        log WARN "Detected Hadoop 3.x ($version), but this script is for 2.x"
        log WARN "Please use the Hadoop 3.x version of this script"
        read -p "Continue anyway? (y/n): " -n 1 -r
        echo
        [[ ! $REPLY =~ ^[Yy]$ ]] && exit 1
    else
        log WARN "Could not detect Hadoop version, assuming 2.x"
    fi
}

# ========== 主程序 ==========
main() {
    log INFO "Starting HDFS Random Fault Injection Script (Hadoop 2.x)"
...
#        fi
        ...
        # 更新统计（使用全局变量）
        if [ $LAST_SELECTED_NODE -ge 0 ] && [ $LAST_SELECTED_NODE -le 4 ]; then
            NODE_FAILURES[$LAST_SELECTED_NODE]=$((NODE_FAILURES[$LAST_SELECTED_NODE] + 1))
...
# 参数检查
if [ "$1" == "--help" ] || [ "$1" == "-h" ]; then
    echo "HDFS Fault Injection Script - Hadoop 2.x Version"
    echo ""
    ...
    echo ""
    echo "Note: This script is designed for Hadoop 2.x"
    echo "      For Hadoop 3.x, use the appropriate version"
    exit 0
fi
