...
# 集群节点配置
CLUSTER_NODES=("ms1108.utah.cloudlab.us" "ms1145.utah.cloudlab.us" "ms1125.utah.cloudlab.us")
...
for namenode in "$NAMENODE1" "$NAMENODE2"; do
    ...
    ssh "$USER@$namenode" "$HADOOP_HOME/sbin/hadoop-daemon.sh stop zkfc"
done
...
for namenode in "$NAMENODE1" "$NAMENODE2"; do
    ...
    ssh "$USER@$namenode" "$HADOOP_HOME/sbin/hadoop-daemon.sh stop namenode"
done
...
for node in "${CLUSTER_NODES[@]}"; do
    ...
    ssh "$USER@$node" "$HADOOP_HOME/sbin/hadoop-daemon.sh stop datanode"
done
...
for jn_node in "${JOURNALNODE_NODES[@]}"; do
    ...
    ssh "$USER@$jn_node" "$HADOOP_HOME/sbin/hadoop-daemon.sh stop journalnode"
done
...
for jn_node in "${JOURNALNODE_NODES[@]}"; do
    ...
    ssh "$USER@$jn_node" "$HADOOP_HOME/sbin/hadoop-daemon.sh start journalnode"
    sleep 2
done
...
ssh "$USER@$NAMENODE1" "$HADOOP_HOME/sbin/hadoop-daemon.sh start namenode"
...
ssh "$USER@$NAMENODE2" "$HADOOP_HOME/sbin/hadoop-daemon.sh start namenode"
...
for namenode in "$NAMENODE1" "$NAMENODE2"; do
    ...
    ssh "$USER@$namenode" "$HADOOP_HOME/sbin/hadoop-daemon.sh start zkfc"
    sleep 3
done
...
for node in "${CLUSTER_NODES[@]}"; do
    ...
    ssh "$USER@$node" "$HADOOP_HOME/sbin/hadoop-daemon.sh start datanode"
    sleep 2
done
...
ssh "$USER@$NAMENODE1" "$HDFS_CMD haadmin -getServiceState nn1"
ssh "$USER@$NAMENODE1" "$HDFS_CMD haadmin -getServiceState nn2"

echo ""
echo "=== Hadoop 2.6 HA集群重置完成 ==="
echo "集群启动完成，请检查以上输出确认所有服务正常运行"
echo ""
echo "常用检查命令："
echo "1. 检查HDFS状态: $HDFS_CMD dfsadmin -report"
echo "2. 检查HA状态: $HDFS_CMD haadmin -getServiceState nn1"
echo "3. 检查HA状态: $HDFS_CMD haadmin -getServiceState nn2"
echo "4. 检查集群健康: $HDFS_CMD dfsadmin -safemode get"
echo "5. 手动切换NameNode: $HDFS_CMD haadmin -transitionToActive nn1"
echo "6. 手动切换NameNode: $HDFS_CMD haadmin -transitionToActive nn2"